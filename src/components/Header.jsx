...
import { Bell, GraduationCap, Search, CheckCircle2, Calendar, FileText, X } from 'lucide-react';
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 bg-white border-b border-slate-200 px-4 lg:px-8 flex items-center justify-between transition-colors font-sans">
      {/* Mobile Brand Title */}
      <Link to={ROUTES.HOME} className="flex items-center gap-2.5 lg:hidden hover:opacity-90 transition-opacity">
        ...
        <span className="font-extrabold text-slate-900 text-base tracking-tight">Placemints</span>
      </Link>
      ...
      <form onSubmit={handleSearchSubmit} className="hidden sm:flex items-center flex-1 max-w-md relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-10 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-600 transition-all font-medium"
        />
        <kbd className="absolute right-3 hidden lg:inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-mono text-slate-400 bg-white border border-slate-200 rounded-md">
          /
        </kbd>
      </form>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-[calc(100vw-2rem)] sm:w-96 max-w-sm bg-white rounded-2xl border border-slate-200 shadow-xl z-50 overflow-hidden animate-fadeIn">
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-xs text-slate-900 uppercase tracking-wider">Campus Notifications</h3>
        ...
        <Link
          to={ROUTES.PROFILE}
          className="flex items-center gap-2.5 hover:opacity-90 transition-opacity p-1 rounded-xl hover:bg-slate-100/60"
        >
          <Avatar 
