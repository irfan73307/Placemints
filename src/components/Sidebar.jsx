...
export function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-screen fixed top-0 left-0 bottom-0 select-none z-40 font-sans transition-colors">
      ...
      <Link
        to={ROUTES.HOME}
        className="h-16 flex items-center px-5 border-b border-slate-200 dark:border-slate-800 gap-3 hover:opacity-90 transition-opacity"
      >
            ...
            return (
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon
                      className={`w-4 h-4 transition-colors ${isActive ? "text-brand-600 dark:text-brand-400" : "text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300"}`}
                    />
                    <span>{item.name}</span>
                  </>
                )}
              ...
              return (
                ...
                >
                  {({ isActive }) => (
                    <>
                      <Icon
                        className={`w-4 h-4 ${isActive ? "text-amber-600 dark:text-amber-400" : "text-slate-400 dark:text-slate-500"}`}
                      />
                      <span>{item.name}</span>
                    </>
                  )}
