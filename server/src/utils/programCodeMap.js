...
const CAMPUS_MAP = {
  ...
  // other campus codes can be added here if/when known
};

function parseRollNumber(emailOrRoll) {
...
function detectBranchFromEmail(email) {
  const parsed = parseRollNumber(email);
  return parsed ? parsed.branch : null;
}

module.exports = { PROGRAM_CODE_MAP, CAMPUS_MAP, parseRollNumber, detectBranchFromEmail };
